return function(obj, ctx)
  ...
  function obj:bindHotkeys()
    ...
    self._hotkeys = {
      hs.hotkey.bind(hk.fix.mods, hk.fix.key, function() self:redraft("fix") end),
      hs.hotkey.bind(hk.improve.mods, hk.improve.key, function() self:redraft("improve") end),
    }
  end

  function obj:unbindHotkeys()
    for _, h in ipairs(self._hotkeys or {}) do h:delete() end
    self._hotkeys = nil
  end
  ...
  function obj:menu()
    ...
    local function prov(kind, name)
      return { title = ctx.LABELS[name] or name, checked = (c[kind .. "Provider"] == name),
               fn = function() self:setProvider(kind, name) end }
    end

    local agents = self._agents or {}
    local agentTool = (c.agent and c.agent.tool) or "auto"
    local agentItems = {
      { title = "Auto (first installed)", checked = (c.improveProvider == "agent" and agentTool == "auto"),
        fn = function() self:setAgent("auto") end },
      { title = "-" },
    }
    for _, t in ipairs(ctx.AGENT_TOOLS) do
      ...
      agentItems[#agentItems + 1] = {
        ...
        disabled = not found,
        fn = function() self:setAgent(t) end,
      }
    end
    agentItems[#agentItems + 1] = { title = "-" }
    agentItems[#agentItems + 1] = { title = "Rescan agents",
      fn = function() self:detectAgents(); self:refreshMenu(); notify("Redraft: rescanned agents") end }
    ...
    improveItems[#improveItems + 1] = { title = "Agent CLI (external: Claude/Codex/Gemini/Copilot)",
      checked = (c.improveProvider == "agent"), menu = agentItems }
    improveItems[#improveItems + 1] = prov("improve", "command")
    if not improveOff then
      improveItems[#improveItems + 1] = { title = "-" }
      improveItems[#improveItems + 1] = { title = "Turn off", fn = function() self:setProvider("improve", "none") end }
    end
    ...
    local items = {
      ...
      { title = self.enabled and "Pause Redraft" or "Resume Redraft",
        fn = function() if self.enabled then self:stop() else self:start() end end },
      { title = "Restart", fn = function() self:restart() end },
      { title = "-" },
      { title = "Fix:  " .. (ctx.LABELS[c.fixProvider] or c.fixProvider), menu = {
        prov("fix", "embedded"),
    ...
    for _, s in ipairs(ctx.STYLES) do
      styleItems[#styleItems + 1] = { title = ctx.STYLE_LABELS[s], checked = (curStyle == s),
        fn = function() self:setImproveStyle(s) end }
    end
    ...
    items[#items + 1] = {
      ...
      checked = preFix,
      fn = function() self:setImprovePreFix(not preFix) end,
    }

    local server = {}
    local function serverItem(kind, provider)
      if c[kind .. "Provider"] ~= provider or not ctx.service.plist(provider) then return end
      local svc, up = ctx.SERVICES[provider], ctx.service.running(provider)
      server[#server + 1] = { title = svc.name .. ":  " .. (up and "● running" or "○ stopped"), menu = {
        { title = "Start", disabled = up, fn = function() self:serviceControl(provider, "start") end },
        { title = "Stop", disabled = not up, fn = function() self:serviceControl(provider, "stop") end },
        { title = "Restart", fn = function() self:serviceControl(provider, "restart") end },
      } }
    end
    ...
    if #server > 0 then
      items[#items + 1] = { title = "-" }
      for _, it in ipairs(server) do items[#items + 1] = it end
    end

    local tail = {
      { title = "-" },
      { title = "Show last result…", disabled = (self._lastResult == nil),
        fn = function() self:showResult() end },
      { title = "Edit config…", fn = function() hs.execute("open '" .. ctx.CONFIG_PATH .. "'") end },
      { title = "Reload config",
        fn = function() self:loadConfig(); self:refreshMenu(); notify("Redraft: config reloaded") end },
      { title = "About Redraft", fn = function() self:showAbout() end },
      { title = "-" },
      { title = "Quit Redraft", fn = function() self:quit() end },
      { title = "-" },
      { title = "⌨  ⌥⌘F Fix  ·  ⌥⌘I Improve", disabled = true },
    }
    for _, it in ipairs(tail) do items[#items + 1] = it end
    return items
  end
  ...
  function obj:refreshMenu()
    ...
    self.menubar:setMenu(function() return self:menu() end)
  end
  ...
  function obj:quit()
    self:unbindHotkeys()
    self.enabled = false
    if self._spinnerTimer then self._spinnerTimer:stop(); self._spinnerTimer = nil end
